...
| &emsp;| | | | 
| --- | --- | --- | --- | 
| Pre-processing | [add_bgcharge_to_inp](#451-add_bgcharge_to_inp) | [replace_xyz_in_inp](#4544-replace_xyz_in_inp) |
| Read/write fch | [extract_noon2fch](#4517-extract_noon2fch) | [fch_mo_copy](#4533-fch_mo_copy) | [fch_u2r](#4534-fch_u2r) |
| Generating guess | [frag_guess_wfn](#4535-frag_guess_wfn) |
| Math operation | [mo_svd](#4542-mo_svd) | [solve_ON_matrix](#4543-solve_on_matrix) | [gvb_sort_pairs](#4536-gvb_sort_pairs) |
...
See relevant Python modules [fch2py](#4546-fch2py), [py2fch](#4547-py2fch), and [fchk](#4549-fchk).
...
To convert MOs from Molpro back to Gaussian, see [xml2fch](#4545-xml2fch).
...
To transfer MOs from (Open)Molcas back to Gaussian, see [orb2fch](#4541-orb2fch).
...
To convert MOs from ORCA to Gaussian, see [mkl2fch](#4538-mkl2fch) and [mkl2gjf](#4539-mkl2gjf). To convert MOs from ORCA to other software packages, you can use `mkl2amo`, `mkl2cfour`, `mkl2com`, `mkl2dal`, `mkl2inp`, `mkl2inporb`, `mkl2mrcc`, `mkl2psi`, `mkl2py`, and `mkl2qchem`.
...
fch2rest h2o.fch -dft 'B3LYP D3'
fch2rest h2o.fch -dft 'B3LYP D3BJ'
fch2rest h2o.fch -dft 'B3LYP D4'
fch2rest h2o.fch -dft 'XYG3'
fch2rest h2o.fch -dft 'XYG7'
fch2rest h2o.fch -dft 'XYGJOS'
```
...
For RI-MP2, you can run either of the following
```
fch2rest h2o.fch -wft MP2
fch2rest h2o.fch -dft MP2
```
Since MOKIT 1.2.8rc2, if the user specifies the MP2 method, the froze-core keyword will be written into the generated file `h2o.in` automatically.


## 4.5.31 fch2tm
Transform MOs from Gaussian to Turbomole, i.e. generate files `control` and `mos` from a .fch(k) file. An example is shown below
```
fch2tm h2o.fch
```
...
You can also transfer MOs from ORCA to Gaussian via [mkl2gjf](#4539-mkl2gjf). To transfer MOs from Gaussian to ORCA, see [fch2mkl](#4525-fch2mkl).
...
To convert MOs from ORCA to Gaussian, also see [mkl2fch](#4538-mkl2fch) and [mkl2gjf](#4539-mkl2gjf). To convert MOs from Gaussian to Turbomole, see [fch2tm](#4531-fch2tm).
...
## 4.5.47 py2fch
Export MOs from PySCF to a Gaussian .fch file. Note that `py2fch` cannot generate a .fch file from scratch. The user must provide one .fch file, in which the 'Alpha Orbital Energies' and 'Alpha MOs' sections (or beta orbital counterpart) will be replaced by occupation numbers and MOs from PySCF. If you want to generate a .fch file from scratch, see the [fchk](#4549-fchk) module.
